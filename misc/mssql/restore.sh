...
do
  ...
  $SQLCMD -S host.docker.internal -C -U sa -P ${MSSQL_SA_PASSWORD} -Q "SELECT 1"
...
if [ -e "/tmp/backup/db.bak" ]; then
  ...
  if [[ $DATABASE_EXISTS != "EXISTS" ]]; then
    ...
    $SQLCMD -S host.docker.internal -C -U $MSSQL_USERNAME -P $MSSQL_SA_PASSWORD -i "/tmp/config/create.sql" || exit 1
  else
    echo "Database exist"
  fi
fi
